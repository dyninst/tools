...
int main(int argc, char** argv) {
   ...
   std::cout << "executable = exec_peach.sh\n";
   std::cout << "notification = Never\n";
   std::cout << "request_memory = 1 GB\n";
   std::cout << "Requirements = ( ( OpSysAndVer == \"RedHat6\") ) || ( ( OpSysAndVer == \"SL6\") )\n";
   
   for (int i = 0; i < 256; i++) {
      std::cout << "transfer_input_files = exec_peach.sh,libstdc++.so.6,libcommon.so.9.1,libbfd-2.26.51.20160204.so,libLLVM-3.9.0svn.so,libopcodes.so,libinstructionAPI.so,libLLVM-3.9svn.so,libxed.so,libinstructionAPI.so.9.1,libLTO.so,peach,libinstructionAPI.so.9.1.0,libopcodes-2.26.51.20160204.so\n";
