...
static bool wouldLlvmPpcInsnSig(char* insn, int nBytes) {
    
    if (nBytes < 4) {
        return false;
    }

    if ((insn[0] == (char)0xf0 || insn[0] == (char)0xf1 || insn[0] == (char)0xf2 || insn[0] == (char)0xF3) &&
        ((insn[3] & (char)0xF0) == (char)0xd0 || (insn[3] & (char)0xF0) == (char)0x90)) {
...
void llvm_ppc_32_norm(char* buf, int bufLen) {
