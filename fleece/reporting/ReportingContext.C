...
ReportingContext::ReportingContext(FILE* outf) {
   ...
   diffCounter = new Hashcounter(RC_HASHCOUNTER_SIZE);
   assert(diffCounter != NULL && "Report hashcounter should not be null!");
...
void ReportingContext::reportDiff(const char** insns, int nInsns, const char* bytes, int nBytes) {
   ...
   assert(fprintf(outFile, "\n") == 1 && "Reporting write failed!");
}

int ReportingContext::processDecodings(const char** insns, int nInsns, const char* bytes, int nBytes) {
...
bool ReportingContext::shouldReportDiff(const char** insns, int nInsns) {
   ...
   for (int i = 0; i < nInsns; i++) {
      ...
      // If they all have the same number of tokens, we can try to line them up
   ...
   /*
    * The snippet below will compare individual tokens and ignore reports where
    * the tokens have already been matched up before.
    */
   ...
   if (nTokens != -1) {
      ...
      // Analyze all tokens produced by each instruction.
      for (int i = 0; i < nTokens; i++) {
         
         // Fille a buffer with the token from each instruction at position i.
         for (int j = 0; j < nInsns; j++) {
            
            char* cur = buf;

            strncpy(cur, tLists[j]->getToken(i), end - cur);
         ...
         // Check if we have seen that combination of tokens before.
         if (diffCounter->increment(buf, "a") < 2) {
            seenBefore = false;
         }
      }

      // If we have seen every single token combination before, we don't want a
      // duplicate report of the issue, so return false.
      if (seenBefore) {
         result = false;
      }
   }
   ...
   if (result == true) {
      ...
      // should be reported.
      result = diffCounter->increment(buf, "") < REPORTING_COUNT_THRESHOLD;
   }
