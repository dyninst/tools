...
static void removeUnused64BitSegRegs(char* buf, int bufLen) {
    static FindList* fl = initUnused64BitSegRegsFindList();
    fl->process(buf, bufLen);
}

static void addImpliedX86Index(char* buf, int bufLen) {
    char* cur = buf;
    bool inParens = false;
    int nCommas = 0;
    while (*cur) {
...
static void removeImplicitST0(char* buf, int bufLen) {
...
static FindList* initMissing0x0FindList() {
    FindList* fl = new FindList(877);
...
static FindList* initOpcodeOnlyMissing0x0FindList() {
    FindList* fl = new FindList(877);
...
static void addMissing0x0(char* buf, int bufLen) {
...
static void removeUnusedOverridePrefixes(char* buf, int bufLen) {
...
static void signOperands(char* buf, int bufLen, void* arg) {
    char* cur = buf;
    char* place;
    while (*cur) {
        if (*cur == '0' && *(cur + 1) == 'x') {
...
static FindList* initFixAddrInsnFindList() {
    FindList* fl = new FindList(877);
...
static void fixAddrInsnSuffix(char* buf, int bufLen) {
    static FindList* fl = initFixAddrInsnFindList();
    fl->process(buf, bufLen);
}

static FindList* initSignedOperandFindList() {
    FindList* fl = new FindList(877);
...
static void signedOperands(char* buf, int bufLen) {
    static FindList* fl = initSignedOperandFindList();
    signOperands(buf, bufLen, NULL);
}

static void removeUnusedRepPrefixes(char* buf, int bufLen) {
    std::string result(buf);
...
static FindList* initFixStRegsFindList() {
    FindList* fl = new FindList(877);
...
static void fixStRegs(char* buf, int bufLen) {
    static FindList* fl = initFixStRegsFindList();
    fl->process(buf, bufLen);
}

static void cleanX86NOP(char* buf, int bufLen) {
    if (strncmp(buf, "nop", 3) == 0) {
        *(buf + 3) = '\0';
    }
}

static FindList* initRemoveHintsFindList() {
    FindList* fl = new FindList(877);
...
static void removeX86Hints(char* buf, int bufLen) {
...
static void convertToHex(char* buf, int bufLen) {
    char tmpBuf[bufLen];
    char* end; 
...
static void decToHexConstants(char* buf, int bufLen) {
   char* cur = buf;
   bool inParens = false;
...
static FindList* initRemoveImplicitK0FindList() {
    FindList* fl = new FindList(877);
    Normalization::addReplaceTerm(*fl, "{%k0}", "");
    return fl;
}

static void removeImplicitK0(char* buf, int bufLen) {
