...
public:
	...
	int FindFuncByLibnameOffset(BPatch_addressSpace * aspace, BPatch_function * & ret,
            std::string libname, uint64_t offset, bool exact = true);
	BPatch_object * FindObjectByName(BPatch_addressSpace * aspace, std::string & name, bool exact = true);
	bool GetFileOffset(BPatch_addressSpace * aspace, BPatch_point * point,
            uint64_t & addr, bool addInstSize = false);
	...
	std::vector<BPatch_function*> FindFunctionsByLibnameOffset(
            BPatch_addressSpace * aspace, std::string libname, uint64_t offset, bool exact);
	...
	std::vector<BPatch_function *> FindFuncsInObjectByName(
            BPatch_addressSpace * aspace, BPatch_object * obj, std::string name);
	std::vector<BPatch_function *> FindFuncsByName(
    ...
    BPatch_point * FindPreviousPoint(BPatch_point* point, BPatch_addressSpace * aspace);
private:
