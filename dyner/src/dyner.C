...
bool haveApp(bool verbose = true)
{
   if (appProc == NULL) {
      if(verbose) fprintf(stderr, "No application loaded.\n");
      return false;
   }
   
   if (appProc->isTerminated()) {
      if(verbose) fprintf(stderr, "The application has exited.\n");
      return false;
   }
...
int instStatement(ClientData, Tcl_Interp *, int argc, TCLCONST char *argv[])
{
   ...
   std::vector<BPatch_point *> *points = func->findPoint(where);
...
void printVarRecursive(BPatch_variableExpr *var, int level)
{
   ...
   if (!strcmp(type->getName(), "int")) {
      ...
      printf("struct {\n");
      level++;
      std::vector<BPatch_variableExpr *> *fields = var->getComponents();
...
int whereMove(bool up){
   ...
   if (up ? (unsigned) whereAmINow < (callStack.size()-1) : whereAmINow > 1 ){
	...
	if(callStack[whereAmINow].findFunction()){
		...
		std::vector<BPatch_point *> *points = callStack[index].findFunction()->findPoint(BPatch_subroutine);
...
{
   if (!haveApp()) return TCL_ERROR;
   std::vector<BPatch_function *> pdfv;
...
int whatisFunc(ClientData, Tcl_Interp *, int, TCLCONST char *argv[])
{
   ...
   if(retType)
      ...
      printf("*unknown*");
   printf(" %s(", argv[1]);
   std::vector<BPatch_localVar *> *params = func->getParams();
...
int whatisType(ClientData cd, Tcl_Interp *interp, int argc, TCLCONST char *argv[])
{
   ...
   switch (dc) {
      case BPatch_structure: {
         printf("    struct %s {\n", argv[1]);
         std::vector<BPatch_field *> *fields = type->getComponents();
         for (unsigned int i=0; i < fields->size(); i++) {
...
void PrintTypeInfo(const char *var, BPatch_type *type) {
   if (!type) {
...
{
   std::vector<BPatch_function *> functions;// = NULL;
   if ((argc != 3) && (argc!=5)) {
      printf("Syntax error !\n");
      return TCL_ERROR;
   }
...
int ShowParameters(int argc, TCLCONST char *argv[]) {
   ...
   std::vector<BPatch_localVar *> *params = fp->getParams();
   for(unsigned int i=0; i<params->size(); ++i) {
...
int ShowLocalVars(const char *funcName) {
   ...
   std::vector<BPatch_localVar *> *vars = fp->getVars();
...
int ShowGlobalVars() {
   std::vector<BPatch_variableExpr *> *vars = appImage->getGlobalVariables();
...
int repFunc(const char *name1, const char *name2) {
   ...
   std::vector<BPatch_function *> bpfv2;
   if (dynerVerbose) printf("Searching for %s...\n", name2);
...
int repCall(const char *func1, const char *func2) {
   ...
   std::vector<BPatch_point *> *points = found_funcs[0]->findPoint(BPatch_subroutine);
